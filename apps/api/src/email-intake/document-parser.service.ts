...
export class DocumentParserService {
  ...
  async parseDocument(attachment: any): Promise<string> {
    this.logger.log(
      `Parsing document: ${attachment.filename} (${attachment.contentType})`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Error parsing document ${attachment.filename}:`,
        error,
      );
  ...
  private async parsePdf(buffer: Buffer): Promise<string> {
    try {
      ...
      if (
        ...
        error.message.includes('cannot be invoked without')
      ) {
        try {
          ...
          if (instance && typeof instance.then === 'function') {
            ...
            return this.enhancePdfText(instance.text || '');
          }
        } catch (retryError) {
