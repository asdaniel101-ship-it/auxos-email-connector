...
export class SubmissionClassifierService {
  ...
  async classify(emailData: any): Promise<{
    ...
    this.logger.log(`Classifying email: subject="${subject || '(empty)'}", body length=${(body || '').length}, attachments=${attachmentsArray.length}`);
    ...
    if (attachmentsArray.length > 0) {
      attachmentsArray.forEach((att: any, idx: number) => {
        ...
        this.logger.log(`  Attachment ${idx + 1}: filename="${filename}", contentType="${contentType}"`);
      });
    } else {
    ...
    const hasRelevantAttachments = attachmentsArray.length > 0 && attachmentsArray.some((att: any) => {
      ...
      const hasInsuranceKeywords = filename.includes('acord') ||
    ...
    const hasSubmissionKeywords =
      submissionKeywords.some((keyword) => subjectLower.includes(keyword) || bodyLower.includes(keyword));

    // Check if "submission" appears anywhere in subject or body (most important indicator)
    const hasSubmissionWord = subjectLower.includes('submission') || bodyLower.includes('submission');
    ...
    if (subjectLower.includes('submission')) {
      ...
      this.logger.log(`Found "submission" in body (length: ${bodyLower.length})`);
    } else {
      this.logger.log(`"submission" NOT found in subject="${subjectLower.substring(0, 50)}..." or body (length: ${bodyLower.length})`);
    }

    // Check if subject contains "New Property Submission" or similar patterns
    const hasSubmissionInSubject = subjectLower.includes('new property submission') ||
    ...
    const isSubmission = hasSubmissionWord ||  // Most important: if "submission" is mentioned, it's a submission
    ...
    if (!isSubmission) {
      ...
      this.logger.log(`Email NOT classified as submission. Reasons: ${reasons.join(', ')}`);
      this.logger.log(`Subject: ${subject}, Attachments: ${attachmentsArray.length}, Body length: ${body?.length || 0}`);
    ...
    let submissionType: 'new_business' | 'renewal' | 'endorsement' | 'other' = 'other';

    if (subjectLower.includes('renewal') || bodyLower.includes('renewal')) {
      submissionType = 'renewal';
    } else if (subjectLower.includes('endorsement') || bodyLower.includes('endorsement')) {
