...
async function bootstrap() {
  ...
  try {
    ...
    logger.error(
      `Missing or invalid environment variables. Check Railway environment variables.`,
    );
  ...
  app.enableCors({
    origin: (origin, callback) => {
      ...
      if (allowedOrigins.includes(origin) || isVercelDomain) {
        ...
        Logger.warn(
          `CORS: Blocked origin: ${origin}. Allowed: ${allowedOrigins.join(', ')}, Vercel domains`,
          'CORS',
        );
  ...
  const config = new DocumentBuilder()
    .setTitle('Auxo API')
    .setDescription(
      'API for restaurant intake, lead management, and partner matching',
    )
    .setVersion('0.1.0')
    .build();
  ...
  logger.log(
    `PORT env var: ${process.env.PORT || 'not set (using default 4000)'}`,
  );
  ...
  try {
    ...
    if (address && typeof address === 'object') {
      if (address.address === '0.0.0.0' || address.address === '::') {
        logger.log(
          `✅ Server is listening on 0.0.0.0:${port} - Railway can reach it`,
        );
      } else {
        logger.warn(
          `⚠️  WARNING: Server address is ${address.address}, expected 0.0.0.0`,
        );
    ...
    logger.log(
      `✅ Health check endpoint available at: http://0.0.0.0:${port}/health/live`,
    );
